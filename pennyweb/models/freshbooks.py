...
""")
    ...
    update_response = c.recurring.update(
        recurring=dict(recurring_id=recurring_id, send_email='1', date=(date.today() + relativedelta(months=1)).strftime('%Y-%m-01')))
    ...
    prorated_lines = [api.types.line(name=l.name, unit_cost=l.unit_cost,
                                     quantity=prorate if l.name in (
                                         'ATXDUES', 'AUTOPAY') else l.quantity,
                                     description=l.description)
                      for l in invoice.lines.line]
    response = c.invoice.update(
        invoice=dict(invoice_id=invoice.invoice_id, lines=prorated_lines, notes='Amount prorated for remainder of month.'))
